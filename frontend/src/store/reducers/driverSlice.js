...
export const getDriversAvailableDataAsync = createAsyncThunk(
  "drivers/getAllDrivers",
  async () => {
    const response = await axios.get(
      `${NEW_BASE_URL}/user/list/driver?status=available`,
      {
        headers: { token: token },
      }
    );
...
// get all drivers
export const getDriversDataAsync = createAsyncThunk(
  "drivers/getAllDrivers",
  async () => {
    const response = await axios.get(`${NEW_BASE_URL}/user/list/driver`, {
      headers: { token: token },
    });
    return response.data.data;
  }
...
// add | post driver
export const postDriverDataAsync = createAsyncThunk(
  "users/drivers/postDriver",
  async (driver) => {
    const response = await axios.post(`${NEW_BASE_URL}/user/driver`, driver, {
      headers: { token: token },
    });
    return response.data.data;
  }
...
// put driver
export const putDriverDataAsync = createAsyncThunk(
  "users/drivers/putDriver",
  async (driver) => {
    const { formData, driverId } = driver;
    const response = await axios.put(
      `${NEW_BASE_URL}/user/info/${driverId}`,
      formData,
      { headers: { token: token } }
    );
...
// delete driver
export const deleteDriverDataAsync = createAsyncThunk(
  "users/drivers/deleteDriver",
  async (driverId) => {
    const response = await axios.delete(`${NEW_BASE_URL}/user/${driverId}`, {
      headers: { token: token },
    });
    return response.data.data;
  }
...
export const getDriverDataById = async (id) => {
  const response = await axios.get(`${NEW_BASE_URL}/user/${id}`, {
    headers: { token: token },
  });
  return response.data.data;
};
...
export const getDriverStateLog = async (driverId) => {
  const response = await axios.get(`/tasks/recent/${driverId}`, {
    headers: { token: token },
  });
  return response.data.data;
};
...
const driversSlice = createSlice({
  ...
  extraReducers: {
    ...
    [putDriverDataAsync.fulfilled]: (state, action) => {
      ...
      const index = state.allDrivers.findIndex(
        (driver) => driver.id === putDriver.id
      );
