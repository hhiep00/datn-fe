...
// get all bins available
export const getBinsAvailableDataAsync = createAsyncThunk(
  "bins/getAllBins",
  async (companyId) => {
    let response = null;
    if (companyId === undefined || companyId === null) {
      response = await axios.get(`${NEW_BASE_URL}/bin/list?status=available`, {
        headers: { token: token },
      });
    } else {
      response = await axios.get(
        `${NEW_BASE_URL}/bin/list?companyId=${companyId}`,
        { headers: { token: token } }
      );
    }
...
// get all bins
export const getBinsDataAsync = createAsyncThunk(
  "bins/getAllBins",
  async (companyId) => {
    let response = null;
    if (companyId === undefined || companyId === null) {
...
// add | post bin
export const postBinDataAsync = createAsyncThunk(
  "bins/postBin",
  async (bin) => {
    const response = await axios.post(`${NEW_BASE_URL}/bin`, bin, {
      headers: { token: token },
    });
    return response.data.data;
  }
...
export const putBinDataAsync = createAsyncThunk("bins/putBin", async (bin) => {
  ...
  const response = await axios.put(`${NEW_BASE_URL}/bin/${binId}`, formData, {
    headers: { token: token },
  });
...
// delete bin
export const deleteBinDataAsync = createAsyncThunk(
  "bins/deleteBin",
  async (binId) => {
    const response = await axios.delete(`${NEW_BASE_URL}/bin/${binId}`, {
      headers: { token: token },
    });
    return response.data.data;
  }
...
export const getBinDataById = async (id) => {
  const response = await axios.get(`${NEW_BASE_URL}/bin/${id}`, {
    headers: { token: token },
  });
  return response.data.data;
};
...
export const getResetBinWeightAsync = createAsyncThunk(
  "bins/getResetBinWeight",
  async (binId) => {
    const response = await axios.get(`/bins/reset/${binId}`, {
      headers: { token: token },
    });
    return response.data.data;
  }
