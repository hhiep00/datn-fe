...
// get all vehicles available
export const getVehiclesAvailableDataAsync = createAsyncThunk(
  "vehicles/getAllVehicles",
  async () => {
    const response = await axios.get(
      `${NEW_BASE_URL}/vehicle/list?status=available`,
      {
        headers: { token: token },
      }
    );
...
// get all vehicles
export const getVehiclesDataAsync = createAsyncThunk(
  "vehicles/getAllVehicles",
  async () => {
    const response = await axios.get(`${NEW_BASE_URL}/vehicle/list`, {
      headers: { token: token },
    });
    // const response = await axios.get("/vehicles", { headers: { token: token } });
    return response.data.data;
...
export const getVehiclesValidationDataAsync = createAsyncThunk(
  "vehicles/validation",
  async () => {
    const response = await axios.get("/vehicles/validation", {
      headers: { token: token },
    });
    return response.data.data;
  }
...
// add | post vehicle
export const postVehicleDataAsync = createAsyncThunk(
  "vehicles/postvehicle",
  async (vehicle) => {
    const response = await axios.post(`${NEW_BASE_URL}/vehicle`, vehicle, {
      headers: { token: token },
    });
...
// put vehicle
export const putVehicleDataAsync = createAsyncThunk(
  "vehicles/putVehicle",
  async (vehicle) => {
    const { formData, vehicleId } = vehicle;
    const response = await axios.put(
      `${NEW_BASE_URL}/vehicle/${vehicleId}`,
      formData,
      { headers: { token: token } }
    );
...
// delete vehicle
export const deleteVehicleDataAsync = createAsyncThunk(
...
export const getRoutesByVehicleId = async (vehicleId) => {
  ...
  const response = await axios.get(
    `${NEW_BASE_URL}/vehicle/route/${vehicleId}`,
    { headers: { token: token } }
  );
  return response.data.data;
};
...
export const getVehicleDataById = async (vehicleId) => {
  const response = await axios.get(`${NEW_BASE_URL}/vehicle/${vehicleId}`, {
    headers: { token: token },
  });
  return response.data.data;
};
...
const vehicleSlice = createSlice({
  ...
  extraReducers: {
    // get all todos
    [getVehiclesDataAsync.pending]: (state, action) => {
      // pending
      console.log("pending");
    },
    [getVehiclesDataAsync.fulfilled]: (state, action) => {
      // fulfilled = success
      console.log("success");
      state.allVehicles = action.payload;
    },
    [getVehiclesDataAsync.rejected]: (state, action) => {
      // rejected = error
      console.log("error");
    },
    ...
    [putVehicleDataAsync.fulfilled]: (state, action) => {
      ...
      const index = state.allVehicles.findIndex(
        (vehicle) => vehicle.id === putVehicle.id
      );
...
export const vehicleValidation = (state) =>
  state.vehiclesReducer.vehicleValidation;
